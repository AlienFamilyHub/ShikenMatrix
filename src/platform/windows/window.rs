...
/// 相当于 DWMWA_USE_IMMERSIVE_DARK_MODE (20)
const DWMWA_USE_IMMERSIVE_DARK_MODE: DWORD = 20;
/// 相当于 DWMWA_SYSTEMBACKDROP_TYPE (38)
const DWMWA_SYSTEMBACKDROP_TYPE: DWORD = 38;
/// DWMSBT_MAINWINDOW (2) - Mica effect
const DWMSBT_MAINWINDOW: DWORD = 2; // Mica
const DWMSBT_TRANSIENTWINDOW: DWORD = 3; // Acrylic

pub fn apply_vibrancy(window: &slint::Window) {
    if let Ok(handle) = window.window_handle().as_ref() {
           ...
           match rwh.as_raw() {
               RawWindowHandle::Win32(handle) => {
                   unsafe {
                       ...
                       // Enable Dark Mode (if system supports)
                       let dark: BOOL = 1;
                       DwmSetWindowAttribute(hwnd, DWMWA_USE_IMMERSIVE_DARK_MODE, &dark as *const _ as LPCVOID, std::mem::size_of::<BOOL>() as u32);
                       
                       // Enable Mica (Windows 11)
                       let backdrop_type: DWORD = DWMSBT_MAINWINDOW;
                       DwmSetWindowAttribute(hwnd, DWMWA_SYSTEMBACKDROP_TYPE, &backdrop_type as *const _ as LPCVOID, std::mem::size_of::<DWORD>() as u32);
                   }
               }
               _ => {}
           }
