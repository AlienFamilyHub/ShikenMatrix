...
export function SettingsPage(props: SettingsPageProps) {
  ...
  const updateNative = (patch: Partial<ReporterConfig['native']>) => {
    props.onConfigChange({ ...props.config, native: { ...props.config.native, ...patch } })
  }
  const updateMixSpace = (patch: Partial<ReporterConfig['mix_space']>) => {
    props.onConfigChange({ ...props.config, mix_space: { ...props.config.mix_space, ...patch } })
  }
  const updateS3 = (patch: Partial<ReporterConfig['s3']>) => {
    props.onConfigChange({ ...props.config, s3: { ...props.config.s3, ...patch } })
  }
      ...
      <div class="settings-shell" ref={shellRef}>
        <section class="settings-section">
          <div class="settings-section-header">
            <h2>上报方案</h2>
            <button class="btn btn-primary save-settings-button" onClick={props.onSave}>
              <IconSave />
              保存设置
            </button>
          </div>
          ...
          <div class="segmented-control" role="tablist" aria-label="上报方案">
            <button
              class={props.config.protocol === 'native' ? 'segment active' : 'segment'}
              onClick={() => updateConfig({ protocol: 'native' })}
            >
              Native
            </button>
            <button
              class={props.config.protocol === 'mix_space' ? 'segment active' : 'segment'}
          ...
          <label class="checkbox-label settings-checkbox">
            <input
              ...
              onChange={event => updateConfig({ enable_media_reporting: event.currentTarget.checked })}
            />
            <span>上报媒体播放信息</span>
          </label>
        </section>
        ...
        <Show
          ...
          fallback={<MixSpaceFields config={props.config} onUpdate={updateMixSpace} onS3Update={updateS3} />}
        >
...
function NativeFields(props: NativeFieldsProps) {
  ...
  onMount(() => {
    gsap.from(sectionRef, { opacity: 0, y: 15, duration: 0.4, ease: 'power3.out' })
  })

  return (
    <section class="settings-section" ref={sectionRef}>
      <h2>Native 协议</h2>
      <p class="settings-note">Native 使用 WebSocket 长连接，窗口与媒体事件分开实时上报。</p>
      <FormInput
        label="WebSocket 地址"
...
function MixSpaceFields(props: MixSpaceFieldsProps) {
  ...
  onMount(() => {
    gsap.from(containerRef.children, { opacity: 0, y: 15, duration: 0.4, stagger: 0.1, ease: 'power3.out' })
  })

  return (
    <div ref={containerRef} style={{ 'display': 'flex', 'flex-direction': 'column', 'gap': '24px' }}>
      <section class="settings-section">
        <h2>Mix-Space 协议</h2>
        <p class="settings-note">Mix-Space 使用上游 ProcessReporter 的 HTTP JSON 结构，将窗口与媒体合并成一次上报。</p>
        <FormInput
          label="Endpoint"
        ...
        <label class="form-group">
          ...
          <select
            ...
            onChange={event => props.onUpdate({ method: event.currentTarget.value })}
          >
  ...
  return (
    <section class="settings-section">
      ...
      <p class="settings-note">
        启用后会先上传应用图标和媒体封面，再分别写入 process.iconUrl 与 media.icon。路径变量支持
        {'{kind}'}
        、
        {'{Y}'}
        、
      ...
      <div class="settings-grid">
        <FormInput label="Bucket" value={props.config.s3.bucket} onInput={value => props.onUpdate({ bucket: value })} />
        <FormInput label="Region" value={props.config.s3.region} onInput={value => props.onUpdate({ region: value })} />
        <FormInput label="Access Key" value={props.config.s3.access_key} onInput={value => props.onUpdate({ access_key: value })} />
        <FormInput label="Secret Key" value={props.config.s3.secret_key} type="password" onInput={value => props.onUpdate({ secret_key: value })} />
      </div>
      <FormInput
        label="对象路径模板"
      ...
      <FormInput
        ...
        type="number"
        placeholder="0"
        onInput={value => props.onUpdate({ lifecycle_days: Number(value) || 0 })}
      />
      <FormInput
        label="Endpoint"
