...
本项目是 [**Kizuna**](https://github.com/AlienFamilyHub/Kizuna) 项目的继任者，在原有功能基础上进行了全面的重构和功能增强，提供更加现代化、高性能和跨平台的用户体验。
...
1. **后端功能**：在 `src/platform/` 中添加平台接口和实现
2. **前端组件**：在对应平台的 UI 项目中创建新的组件
3. **FFI 桥接**：在 `src/ffi/` 中添加 C 接口，前端通过 `RustBridge` 调用
...
根据 [@My-Iris](https://github.com/Mx-Iris) 的发现，bundle identifier 以 `com.apple.` 开头的进程被授予访问 MediaRemote 框架的权限。系统 Perl 二进制文件 `/usr/bin/perl` 的 bundle identifier 为 `com.apple.perl`，因此被系统信任。
...
2. **Perl 沙盒层**
   - 使用 Perl 的 `DynaLoader` 动态加载 dylib
   - 绕过 macOS 15.4+ 的 entitlement 验证（Perl 进程不受限制）

3. **objc2 直接绑定**
   - 通过 `objc2` 直接调用 MediaRemote Framework
   - 使用 `MRMediaRemoteGetNowPlayingInfo` 获取播放信息
   - 解析 NSDictionary 获取元数据和封面
...
| 系统 MP* 框架 | ⚠️ 仅 Apple Music | ❌ 不需要 | 低（官方） |
...
- [objc2](https://github.com/madsmtm/objc2) - Objective-C 绑定
- [tokio](https://tokio.rs/) - 异步运行时
- [SwiftUI](https://developer.apple.com/xcode/swiftui/) - 原生 UI 框架
- [mediaremote-adapter](https://github.com/ungive/mediaremote-adapter) - Perl 沙盒层设计思路参考
- [MediaRemote-rs](https://github.com/TNXG/MediaRemote-rs) - macOS MediaRemote 框架绑定（自有库）
- [WinUI 3](https://developer.microsoft.com/windows/apps/winui/) - Windows 原生 UI 框架
