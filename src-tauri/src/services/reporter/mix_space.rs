...
impl Reporter {
    pub(super) async fn run_mix_space_reporter(
        config: Arc<RwLock<ReporterConfig>>,
        mut rx: tokio::sync::mpsc::UnboundedReceiver<ReporterMessage>,
...
async fn send_mix_space_payload(
    ...
    if config.mix_space.endpoint.trim().is_empty() {
        Reporter::set_last_error(last_error, "Mix-Space endpoint is empty");
        return;
    }

    let icon_url = resolve_app_icon_url(client, &config, window, icon_urls, last_error).await;
    ...
    match result {
        Ok(_) => Reporter::clear_last_error(last_error),
        Err(error) => {
            Reporter::set_last_error(last_error, format!("Mix-Space report failed: {error}"));
        }
    }
}
...
async fn resolve_app_icon_url(
    ...
    let icon_data = match BASE64.decode(icon_base64) {
        Ok(data) => data,
        Err(error) => {
            Reporter::set_last_error(last_error, format!("Decode app icon failed: {error}"));
            return None;
        }
    };
    ...
    match s3::upload_app_icon(client, config, &icon_data, &window.process_name).await {
        ...
        Err(error) => {
            Reporter::set_last_error(last_error, format!("S3 app icon upload failed: {error}"));
            None
        }
    }
}
...
async fn upload_media_icon(
    ...
    if media_icon_urls.contains_key(&content_item_identifier) {
        return;
    }
    ...
    match s3::upload_media_icon(client, &config, &artwork_data, &mime_type).await {
        ...
        Err(error) => {
            Reporter::set_last_error(last_error, format!("S3 media icon upload failed: {error}"));
        }
    }
}
