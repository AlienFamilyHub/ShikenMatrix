...
use mediaremote_rs::{NowPlayingInfo, get_now_playing, is_playing};
...
fn update_cache_from_info(info: &NowPlayingInfo, cache: &mut MediaCache) {
    ...
    // 封面数据更新逻辑：
    // 1. 如果歌曲没变（key 相同）且已有缓存，优先复用缓存（避免重复分配）
    // 2. 如果有新的封面数据，直接缓存 mediaremote-rs 0.2.0 返回的二进制数据
    // 3. 否则清空封面
    let (artwork_data, artwork_mime_type) = if !key_changed
        && cache
            .metadata
            .as_ref()
            .and_then(|m| m.artwork_data.as_ref())
            .is_some()
    {
        // 歌曲没变，复用缓存的封面
        let cached_artwork = cache.metadata.as_ref().and_then(|m| m.artwork_data.clone());
        let cached_mime = cache
            .metadata
            .as_ref()
            .and_then(|m| m.artwork_mime_type.clone());
        (cached_artwork, cached_mime)
    } else if let Some(artwork_bytes) = &info.artwork_data {
        // mediaremote-rs 0.2.0 已经返回原始二进制封面，不再需要 Base64 解码。
        cache.artwork_key = Some(new_artwork_key.clone());
        (
            Some(Arc::new(artwork_bytes.clone())),
            info.artwork_mime_type.clone(),
        )
    } else {
    ...
    cache.playback_state = Some(PlaybackState {
        ...
        playback_rate: info
            .playback_rate
            .unwrap_or(if info.playing { 1.0 } else { 0.0 }),
