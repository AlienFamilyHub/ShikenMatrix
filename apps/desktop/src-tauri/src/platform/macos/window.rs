            ...
            if let Some(ref guard) = cache_guard {
                if let Some(ref cache) = **guard {
                    if cache.last_pid == pid
                        && cache.last_update.elapsed()
                            < Duration::from_millis(WINDOW_CACHE_DURATION_MS)
                    {
...
fn get_app_icon_png(app: &NSRunningApplication) -> Option<Vec<u8>> {
    ...
    unsafe {
        ...
        let cg_image = icon.CGImageForProposedRect_context_hints(&mut proposed_rect, None, None)?;
