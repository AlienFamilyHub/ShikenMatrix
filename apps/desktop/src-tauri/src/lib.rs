...
use services::{
    CloseBehavior, Monitor, Reporter, ReporterConfig, load_config, save_close_behavior,
    save_reporter_config,
};
...
fn start_monitor(
    ...
    let monitor = Monitor::new(Some(tx));
...
fn setup_app_menu(app: &mut tauri::App) -> tauri::Result<()> {
    ...
    {
        let about_item =
            MenuItemBuilder::with_id("navigate_about", "关于 ShikenMatrix").build(app)?;
