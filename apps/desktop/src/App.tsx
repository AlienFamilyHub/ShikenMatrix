...
export default function App() {
  ...
  const [permissions, setPermissions] = createSignal<PermissionStatus>({ accessibility: false, media: false, accessibility_required: false });
  ...
  const toggleReporter = async () => {
    ...
    if (!isReporterConfigValid(config())) {
      addLog("Error", "配置无效：请检查 Server WebSocket 地址");
      setPage("settings");
      return;
    }
...
function isReporterConfigValid(config: ReporterConfig): boolean {
  return config.server.ws_url.trim().length > 0;
}
